...
module type Pool_sink = sig
  ...
  val create :
    ...
    -> pool
    -> Logger.t
    -> 'wrapped_t Strict_pipe.Reader.t * t

  (* Sink that ignores all messages *)
  val void : t

  val rate_limiter : t -> Rate_limiter.t
end
...
module Base
  ...
  (* keep logger in type *)
  let verify_impl resource_pool rl logger env cb :
      Diff.verified Envelope.Incoming.t option Deferred.t =
    (* trace_recurring (Diff.label ^ "_verification") (fun () -> *)
    (* let diff, cb = f d in *)
    if BC.is_expired cb then Deferred.return None
      ...
      | `Capacity_exceeded ->
          ...
          BC.error (Error.of_string "exceeded capacity") cb >>| fun _ -> None
      | `Within_capacity -> (
  ...
  let push t (msg, cb) =
    match t with
    | Sink { writer = w; logger; rate_limiter = rl; pool; wrap } -> (
        let env' = Msg.convert msg in
        let cb' = Msg.convert_callback cb in
        match%bind verify_impl pool rl logger env' cb' with
        | None ->
            (* TODO log unverified? *)
            Deferred.unit
        | Some verified_env ->
  ...
  let create ~wrap ~unwrap pool logger =
    let r, writer =
    ...
    in
    (r, Sink { writer; logger; rate_limiter; pool; wrap })

  let void = Void

  let rate_limiter = function
    | Sink { rate_limiter; _ } ->
        rate_limiter
    | Void ->
        failwith "rate_limiter method not supported for void sink"
end
