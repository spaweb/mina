...
(* let handle_v0_case sink env vc m =
  Strict_pipe.Writer.write sink
    ( Envelope.Gossip
        { incoming = Envelope.Incoming.map ~f:(fun _ -> m) env
        ; topic = v0_topic
        }
    , vc ) *)
...
let empty_publish_functions =
  ...
  { publish_v0 = unit
  ; publish_v1_block = unit
  ; publish_v1_tx = unit
  ; publish_v1_snark_work = unit
  }
